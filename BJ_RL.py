...
BOARDSLOTS = 64 # how many icon in this gameboard
...
def exploreCoord(grid, i, j):
    ...
    #explore below
    for row in range(i+1, BOARDHEIGHT):
        if color != grid[row, j]:
            break
    ...
    #explore right
    for col in range(j+1, BOARDWIDTH):
        if color != grid[i, col]:
            break
...
def isValidCoord(coord):
    if (coord[0] < 0 or coord[0] >= BOARDHEIGHT) or (coord[1] < 0 or coord[1] >= BOARDWIDTH):
        return False
    else:
        return True
...
#         print(rowCount)
        max_row.append(max(rowCount[(i, color)] for color in range(1, NUMGEMIMAGES)))
    for j in range(BOARDWIDTH):
        for i in range(BOARDHEIGHT):
            colCount[(j, grid[i,j])] += 1
        max_col.append(max(colCount[(j, color)] for color in range(1, NUMGEMIMAGES)))
    return max_row + max_col

def getFeatureVec(state):
    state = np.array(state)
    nValidMoves = numValidMoves(state)
    phi = [nValidMoves]
#     print(phi)
    phi += maxCount(state)
...
def runGame(rounds, isTrain = True):
    step = 0
    global cur
    for episode in range(rounds):
        ...
        while True: # main game loop
            ...
            reward = 0
            cur_state = gameBoard.copy()
            posible_actions = actions(cur_state)
            cur_state = getFeatureVec(cur_state)
            action = RL.act(cur_state, posible_actions)

            icon = np.argmax(action[0])
            print(icon)
            icon_loc = icon // 2
            orgX = icon_loc % BOARDWIDTH
            orgY = icon_loc // BOARDHEIGHT

            movement = icon % 2

            firstSelectedGem = {'x': orgX,
                                'y': orgY}
            clickedSpace = None
            # select up or down dosen't matter, so do right and left
            invalid = False
            if movement == 0: # down
                if orgY == BOARDHEIGHT-1:
                    invalid = True
                else:
                    clickedSpace = {'x': orgX,
                                    'y': orgY + 1}                      
            elif movement == 1: # right
                if orgX == BOARDWIDTH-1:
                    invalid = True
                else:
                    clickedSpace = {'x': orgX + 1,
                                    'y': orgY} 
...
#       Environment interaction
            ...
            # Two gems have been clicked on and selected. Swap the gems.
                # print(firstSelectedGem, clickedSpace)
                try:
                ...
                if count_miss == 30:
                    print('Final score: ',score)
                    print('Continuously make 30 mistakes')
                    print('Epsilon: ', RL.epsilon)
                    gameIsOver = True
                    reward -= 50
                    RL.save_model()
...
#  =================================================================
            ...
            new_state = gameBoard
            new_state = getFeatureVec(new_state)
            done = gameIsOver

            if step > 30 and step % 5 == 0 and isTrain:
                RL.train()
            print('Step reword: ', reward)
            RL.remember(cur_state, action, reward, new_state, done)
