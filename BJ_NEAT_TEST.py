...
m = Condition(BOARDWIDTH * BOARDHEIGHT)
def main(gamename = 'Gemgem'):
...
def runGame():
    ...
    global gameBoard, score, count_miss
    ...
    #     orgX = np.random.randint(7)
    #     orgY = np.random.randint(7)
    #     movement = np.random.randint(2)
...
#       Environment interaction
    ...
    return cur_state, action, reward, new_state, done

def interact_grid(B, action):
    global gameBoard, score, count_miss
    gameBoard = B
    score = 0
    count_miss = 0
...
if __name__ == '__main__':
    ...
    runGame()
    while True:
        cur_state, action, reward, new_state, done = interact()
        if done:
            break